...
public class ValidateRequestImpl implements ValidateRequest {
    ...
    @Override
    public boolean validateRequest(AuthUserRequestDto requestDto) {
    ...
    private boolean isValidEmail(String email) {
        ...
        if (isLengthNotValid(email, 5, 20) ||
                isNotMatches(email, regexPatternEmail)) {
            throw new InvalidRequestDataException(
                    String.format("Email is not correct: %s. For example: example@gmail.com", email));
        }
        return true;
    }

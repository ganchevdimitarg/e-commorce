...
- 2026-07-01 — auth/profile: split the shared Mongo `users` collection by ownership — auth owns credentials (Postgres `user_credentials`), profile owns personal data (Mongo `profiles`), linked only by `userId` via Kafka choreography (`auth.user.registered` / `auth.user.deleted`). Registration and deletion are auth-owned; profile consumes the events. See `docs/sagas/user-registration.md`.
